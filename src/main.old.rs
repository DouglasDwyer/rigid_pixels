...
impl ObjectIntegrator {
    fn schedule_steps(&mut self, event: &on::PhysicsUpdate) {
        ...
        println!("{delta_time} for {required_steps}");
...
fn setup_scene(ctx: &mut GameWorld) {
    ...
    //ctx.objects.insert(create_rectangle_body(vec2(25.0, 20.5), 0.0, uvec2(15, 17)));
    ctx.objects.insert(create_circle_body(vec2(-20.0, 10.0), 0.2, 9.0));
    //ctx.objects.insert(create_rectangle_body(vec2(-5.0, 2.0), 0.0, uvec2(3, 3)));*/
    
    ctx.objects.insert(create_floor2());
    ctx.objects.insert(create_rectangle_body(vec2(25.0, 20.5), 0.0, uvec2(4, 3)));
    ctx.objects.insert(create_rectangle_body(vec2(20.0, 22.5), 0.0, uvec2(3, 5)));
    ctx.objects.insert(create_rectangle_body(vec2(15.0, 20.5), 0.0, uvec2(4, 7)));
    ctx.objects.insert(create_rectangle_body(vec2(25.0, 26.5), 0.0, uvec2(6, 2)));
    ctx.objects.insert(create_rectangle_body(vec2(30.0, 20.5), 0.0, uvec2(3, 3)));
}
...
async fn main() {
    loop {
        ...
        loop {
            ...
            while next_physics_time < start_time {
                ctx.flush()
